...
# Convert any non-string values to strings before applying the lambda
remove_slash = lambda s: str(s).replace('/', ' ') if pd.notna(s) else s
...
replace_parenthesis = lambda s: s.replace('(', ' ').replace(')', ' ').replace('{', ' ').replace('}', ' ').replace('[', ' ').replace(']', ' ').replace
remove_tabs = lambda s: s.replace('\t', ' ')
remove_newlines = lambda s: s.replace('\n', ' ')
remove_rlines = lambda s: s.replace('\r', '')
remove_period_and_quote = lambda s: (s.replace('.', '').replace('"', '').replace(";", "").replace("'", "").replace(",", "").replace("(", "").replace(")", "").replace(":", "")
                                     .replace("*", ""))
...
# Apply transformations to df1['review']
...
# Display the first few rows of the data and save to a CSV file
df1.to_csv(r'Cleaned_Data\UCIdrug_train.csv', index=False)


# Apply transformations to df2['review']
